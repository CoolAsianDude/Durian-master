...
export default function UserManage() {
  ...
  const updateUserRole = async (userId: string, newRole: string) => {
    try {
      const data = await apiFetch(`/admin/users/${userId}/role`, 'PUT', { role: newRole });
      if (data.success) {
  ...
  const confirmDeactivateUser = async () => {
    ...
    try {
      const data = await apiFetch(
        `/admin/users/${userToDeactivate._id}/deactivate`,
        'PUT',
  ...
  const reactivateUser = async (userId: string) => {
    try {
      const data = await apiFetch(`/admin/users/${userId}/activate`, 'PUT');
      if (data.success) {
  ...
  // Filtered list
  // ---------------------------
  const displayedUsers = showDeactivated ? users : users.filter(u => u.isActive);
  ...
  if (loading) {
    return (
      <View style={styles.centeredContainer}>
        ...
        <Text style={{ marginTop: 12, fontFamily: Fonts.medium, color: Palette.slate }}>
          Loading users...
        </Text>
      </View>
    );
  ...
  return (
    <View style={{ flex: 1, flexDirection: (isSmallScreen || !isWeb) ? 'column' : 'row', backgroundColor: Palette.linenWhite }}>
      ...
      {sidebarVisible && (isSmallScreen || !isWeb) && (
        <TouchableOpacity style={overlayStyles.overlay} activeOpacity={1} onPress={() => setSidebarVisible(false)} />
      )}
        ...
        <ScrollView style={styles.container} contentContainerStyle={(isSmallScreen || !isWeb) ? undefined : { paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.card, { marginBottom: 40 }]}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <TouchableOpacity style={[styles.retryBtn, { alignSelf: 'flex-start' }]} onPress={() => setShowDeactivated(prev => !prev)}>
              ...
              displayedUsers.map(user => (
                <View key={user._id} style={styles.userRow}>
                  ...
                  <View style={styles.userActions}>
                    <View style={styles.pickerWrapper}>
                      <Picker
                        ...
                        onValueChange={(itemValue) => {
                          if (itemValue === 'deactivate') openDeactivateModal(user);
                          else updateUserRole(user._id, itemValue);
                        }}
                        style={styles.picker}
                        mode="dropdown"
                      >
                    ...
                    {!user.isActive ? (
                      <TouchableOpacity onPress={() => reactivateUser(user._id)} style={styles.retryBtn}>
                        <Text style={styles.retryBtnText}>Reactivate</Text>
                      </TouchableOpacity>
                    ) : (
                      <TouchableOpacity onPress={() => openDeactivateModal(user)} style={styles.deleteBtn}>
            ...
            <View style={modalStyles.modalContainer}>
              ...
              <View style={modalStyles.buttonRow}>
                ...
                <TouchableOpacity style={[modalStyles.confirmBtn, deactivating && modalStyles.disabledBtn]} onPress={confirmDeactivateUser} disabled={deactivating}>
                  <Text style={modalStyles.confirmBtnText}>{deactivating ? 'Wait...' : 'Confirm'}</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
...
const modalStyles = RNStyleSheet.create({
  ...
  modalContainer: { backgroundColor: Palette.white, borderRadius: 24, padding: 24, width: '100%', maxWidth: 400, elevation: 10 },
  modalTitle: { fontSize: 20, fontFamily: Fonts.bold, color: Palette.charcoalEspresso, marginBottom: 8, textAlign: 'center' },
  userInfo: { fontSize: 14, color: '#666', marginBottom: 16, textAlign: 'center' },
  label: { fontSize: 14, fontFamily: Fonts.semiBold, marginBottom: 8 },
  textInput: { borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 12, minHeight: 80, backgroundColor: '#f9f9f9', marginBottom: 20 },
  buttonRow: { flexDirection: 'row', gap: 12 },
  cancelBtn: { flex: 1, backgroundColor: '#e0e0e0', padding: 12, borderRadius: 8, alignItems: 'center' },
  cancelBtnText: { fontFamily: Fonts.semiBold },
  confirmBtn: { flex: 1, backgroundColor: '#d32f2f', padding: 12, borderRadius: 8, alignItems: 'center' },
  confirmBtnText: { color: Palette.white, fontFamily: Fonts.bold },
  disabledBtn: { backgroundColor: '#ccc' },
});
const mobileHeaderStyles = RNStyleSheet.create({
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 15, backgroundColor: Palette.white, borderBottomWidth: 1, borderBottomColor: '#eee' },
  title: { fontSize: 18, fontFamily: Fonts.bold, color: Palette.deepObsidian },
});